...
namespace BNP_Plugins
{
    ...
    public void Execute(IServiceProvider serviceProvider)
    {
      ...
      if (CRMData.ValidateTargetAsEntity("opportunity", context))
      {
        try
        {
          ...
          GetAndSetBPFStageName(opportunity, null, iOrganizationService, iTracingService);
        }
            ...
            entityStageId != Guid.Empty)
        {
          ...
          if (processStageEntityCollection.Entities.Count > 0)
          {
            ...
            if (string.IsNullOrWhiteSpace(entityStageName) == false)
            {
              ...
              CRMData.AddAttribute<string>(entity, "stepname", entityStageName);
              iTracingService.Trace("entity stepname updated.");
            }
          }
        }
      }
  ...
  public class PreOpportunityUpdate : IPlugin
  {
    ...
    private readonly string preImageAlias = "PreOpportunityUpdatePreImage";
    #endregion
    ...
    public void Execute(IServiceProvider serviceProvider)
    {
      ...
      if (CRMData.ValidateTargetAsEntity("opportunity", context))
      {
        try
        {
          ...
          PreOpportunityCreate.GetAndSetBPFStageName(opportunity, opportunityRetrieved, iOrganizationService, iTracingService);
        }
        catch (FaultException<OrganizationServiceFault> ex)
        {
          throw new InvalidPluginExecutionException("An error occurred in the PreLeadCreate plug-in." + ex.Message);
