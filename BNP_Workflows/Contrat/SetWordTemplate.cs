      ...
      if (contractProgramme != null && contractTVA != null)
      {
        ...
        if (mappingProgrammeDocumentEntityCollection.Entities.Count > 0)
        {
          ...
          if (string.IsNullOrWhiteSpace(documentTemplateId) == false)
          {
            ...
            //Check the version of the CRM to select the appropriate workflow XAML.
            var verRequest = new RetrieveVersionRequest();
            RetrieveVersionResponse verResponse = (RetrieveVersionResponse)service.Execute(verRequest);
            var crmVersion = new Version(verResponse.Version);
            tracingService.Trace("crmVersion:{0}", crmVersion.ToString());

            string mainXaml;

            if (crmVersion >= new Version("8.2"))
            {
              #region Create XAML
